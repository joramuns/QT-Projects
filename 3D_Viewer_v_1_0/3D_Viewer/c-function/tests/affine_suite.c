...
static float degreesToRadiand(float value) { return (value * S21_PI) / 180; }

static const float original_array[] = {-1.480759, 1.795634,  1.982020,  1.0,
                                       -1.097703, -2.336702, -0.361380, 1.0,
                                       -4.594137, 3.352988,  -1.273126, 1.0};

static float *get_copy() {
  float *copy = calloc(ARR_SIZE, sizeof(float));
  for (int i = 0; i < ARR_SIZE; i++) copy[i] = original_array[i];
...
START_TEST(rotate_x_5) {
  ...
  float expected[] = {-1.480759, 1.961545,  1.817978,  1.0,
...
START_TEST(rotate_counter_x_5) {
  float *array = get_copy();
  turn_matrix_x(degreesToRadiand(5), array, ARR_SIZE);
  float expected[] = {-1.480759, 1.616057,  2.130978,  1.0,
...
START_TEST(rotate_y_5) {
  ...
  float expected[] = {-1.647869, 1.795634,  1.845421,  1.0,
...
START_TEST(rotate_counter_y_5) {
  float *array = get_copy();
  turn_matrix_y(degreesToRadiand(5), array, ARR_SIZE);
  float expected[] = {-1.30238,  1.795634,  2.103534,  1.0,
...
START_TEST(rotate_z_5) {
  ...
  float expected[] = {-1.318624, 1.917858,  1.98202,   1.0,
...
START_TEST(rotate_counter_z_5) {
  float *array = get_copy();
  turn_matrix_z(degreesToRadiand(5), array, ARR_SIZE);
  float expected[] = {-1.631624, 1.659744,  1.98202,   1.0,
...
START_TEST(movement_xyz_5) {
  ...
  float expected[] = {3.519241, 6.795634, 6.98202,  1.0,
...
START_TEST(movement_counter_xyz_5) {
  float *array = get_copy();
  coordinate_change(-5, -5, -5, array, ARR_SIZE);
  float expected[] = {-6.480759, -3.204366, -3.01798,  1.0,
...
START_TEST(scale_positive) {
  ...
  float expected[] = {-2.961518, 3.591268,  3.964040,  1.000000,
...
START_TEST(scale_negative) {
  ...
  float expected[] = {-0.148076, 0.179563,  0.198202,  1.000000,
