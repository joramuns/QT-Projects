...
void Parser::AddPointInArray() noexcept {
  if (!vertex_points_.empty()) all_vertices_.push_back(vertex_points_);
  if (!texture_points_.empty()) all_textures_.push_back(texture_points_);
  if (!normal_points_.empty()) all_normals_.push_back(normal_points_);
};
