...
namespace s21 {
/// @brief Родительский абстрактный класс, который описывает базовое поведение для всех наследников.
class FacesStrategy {
public:
  ...
  /// @brief Core функция, выполняющая считывание данных и сохранение, для дальнейшего использования
  /// @return Позицию в потоке на которой закончилось считывание
  virtual int Pars() = 0;

protected:
  /// @brief Функция отвечает за теселяцию индексов полигонов, организовывает правильный порядок индексов в векторе
...
protected:
  std::ifstream *file_; ///< Файловый поток, из которого происходит чтение данных

  std::vector<GLint> vertices_; ///< Целевой(преобразованный) вектор индексов вершин
  std::vector<GLint> texutres_; ///< Целевой(преобразованный) вектор индексов текстур
  std::vector<GLint> normals_; ///< Целевой(преобразованный) вектор индексов нормалей
...
/// @brief Класс наследник, обеспечивающий работу с индексами ВЕРШИН, ТЕКСТУР и НОРМАЛЕЙ
