...
Данный файл содержит в себе определения класса парсера, считывающего и сортирующего
данные из файла obj формата, для работы с ними в  OpenGL
*/
...
public:
  ...
  /// @brief Аксессор
  /// @return Вектор отсортированных данных для отрисовки 3-х мерной модели в OpenGL
...
private:
  ...
  /// @brief Заполняет all_vertices_ несоритрованными данными считанными из потока 
  void AddPointInArray() noexcept;

  /// @brief Отчищает промежуточные буферы, служащие для заполнения при считывании
  ...
  /// @param current_position Позиция в потоке с которой необходимо начать считывание
  void SetStrategy(std::ifstream *file, int current_position) noexcept;

  /// @brief Заполнение вектора неотсортированных данных из структуры заполненной данными из потока
  /// @param vertices_struct Структура координат вершин
  void StructFill(const PointCoordinates &vertices_struct) noexcept;

  /// @brief Заполнение вектора координат текстур неотсортированными данными из структуры заполненной данными из потока
  /// @param textures_struct Структура координат текстур
  void StructFill(const TexturesCoordinates &textures_struct) noexcept;

  /// @brief Заполнение вектора координат нормалей неотсортированными данными из структуры заполненной данными из потока
  ...
  /// @brief Сортирует и упаковывает данные в один вектор для дальнейшей отрисовки 3-х мерной модели в OpenGL
  void Packer() noexcept;

  // void DebugPrint() noexcept;

private:
  /// @brief Вектор координат вершин для одного 3-х мерного объекта(возможна мультиобъектная модель)
  std::vector<GLfloat> vertex_points_;

  /// @brief Вектор координат текстур для одного 3-х мерного объекта(возможна мультиобъектная модель)
  std::vector<GLfloat> texture_points_;

  /// @brief Вектор координат нормалей для одного 3-х мерного объекта(возможна мультиобъектная модель)
  ...
  /// @brief Вектор векторов индексов всех координат вершин для 3-х мерной модели
  std::vector<std::vector<GLint>> vertex_faces_;

  /// @brief Вектор векторов индексов всех координат текстур для 3-х мерной модели
  std::vector<std::vector<GLint>> texture_faces_;

  /// @brief Вектор векторов индексов всех координат нормалей для 3-х мерной модели
